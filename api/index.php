...
$pdo = new PDO('mysql:host=localhost;dbname=test;charset=utf8', 'oot', '');
...
switch($action) {
    ...
    case 'actualizar':
        header('Content-Type: application/json');
        $data = json_decode(utf8_encode(file_get_contents("php://input")), true);
        print_r(json_encode(actualizar($fluent, $data)));
        break;
    case 'eliminar':
        header('Content-Type: application/json');
...
function actualizar($fluent, $data)
{
    $id = $data['id'];
    unset($data['id']);
            
    $fluent->update('empleado', $data, $id)
           ->execute();
    echo "Hello";
    return true;
}
